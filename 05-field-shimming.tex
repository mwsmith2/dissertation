...
The storage ring was also engineered with several built-in knobs to the tune the field locally in azimuth.  Each of the shims was tailored to have leverage on different complementary aspects of the field. Figure \ref{fig:shim-cross-section-view} gives a visual overview of the shims.
...
\caption{
    Diagrams and images highlighting the pole feet.  The top image shows full poles which have pole feet at five locations at the interface between the pole and the yoke.  The bottom image shows pole feet of two adjacent pole pieces closer.  The diagram depicts the shimming method uses to adjust the poles.  The feet were removed and a stack of thin metal shimstock pieces was inserted to to elevate that region of the pole surface. 
...
The primary tool used in the rough shimming process was a mobile platform engineered from low-magnetism materials to make measurements throughout the muon storage region by using the lower pole surface as track.  The tool was called the ``shimming cart'' and a rendered model of the shimming cart is shown in figure \ref{fig:shimming-cart}.  The cart was equipped with magnetometers, thermometers, distance sensors, and laser tracker beacons to measure the magnetic field, the local temperature, the pole gap and relay its relative position in the storage ring.
...
\begin{figure}
\centering
\includegraphics[width=0.6\linewidth]{fig/ring-leveling-field-effects.png}
\caption{
...
\caption{
    ...
    \label{fig:pole-moves-field-comparTison}
}
\end{figure}
...
Several prototype steel shims were ordered for calibration and proof of principle testing of edge shim adjustments.  One prototype edge shim of each type (inner lower, outer lower, inner upper, and outer upper) was installed in the place of the current edge shim one different poles around the ring.  The difference in thickness was measured and used to calibrate the multipole effects of the edge shims.  The results are shown figure \ref{fig:edge-shim-model}. In addition to E821 style legwork, the E989 team set up some tests using capton as spacer under edge shims in lieu of grinding down shims and some tests using steel shimstock in lieu of implementing thicker edge shims.
...
David Kawall, a Professor at UMass Amherst, implemented the model for optimization using iron laminations.  The model treated the metal strips as thin, fully magnetized iron dipoles with several orders of magnetic images manifesting inside the pole piece.  The model split each pole into 41 azimuthal sections and 3 radial sections to give a lever on both higher field asymmetries and variations of the average field localized in azimuth.  
