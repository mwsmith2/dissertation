...
\noindent
The ideal energy threshold for this figure of merirt is $0.4\times$\pmagic$ = $ \SI{1.24}{\GeV/c} if all electrons could be effectively captured.  When detector acceptance is folded in, the optimal energy threshold becomes $0.6\times$\pmagic$ = $ \SI{1.86}{\GeV/c} \cite{e989-tdr}.  The connection of muon spin to the decay electron energy is now established, so the last step in measuring the spin precession requires a technique to measure the energy and the birth time of emitted electons. \cite{e821-prd}
